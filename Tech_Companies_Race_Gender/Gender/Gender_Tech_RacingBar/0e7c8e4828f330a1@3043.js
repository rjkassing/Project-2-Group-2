export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["2018_2014_tech_div_female@1.csv",new URL("./files/4909bba99b3941765a43414bb88ac420edaabe07f5f59e7ca81cf7d47e161af0c197abb3405541a77bf67a2754eeb181c75be2d05d5853bc37f13e8a19dedb9f",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  
  //main.variable(observer()).define(["md"], function(md){return(md)});
  
  main.variable(observer("data")).define("data", ["d3","FileAttachment"], async function(d3,FileAttachment){return(
d3.csvParse(await FileAttachment("2018_2014_tech_div_female@1.csv").text(), d3.autoType)
)});

  //main.variable(observer()).define(["md"], function(md){return(md)});

  main.variable(observer("viewof replay")).define("viewof replay", ["html"], function(html){return(html`<button>Replay`)});
  main.variable(observer("replay")).define("replay", ["Generators", "viewof replay"], (G, _) => G.input(_));
  //main.variable(observer("title")).define("title", ["md"], function(md){return(md)});
  main.variable(observer("chart")).define("chart", ["replay","d3","width","height","bars","axis","labels","ticker","keyframes","duration","x","invalidation"], async function*(replay,d3,width,height,bars,axis,labels,ticker,keyframes,duration,x,invalidation)
{
  ...
  /////////////duration time/////////////
  main.variable(observer("duration")).define("duration", function(){return(350)});
  ///////////////////////////////////////
  ...
  //main.variable(observer()).define(["data"], function(data){return(data)});
  //main.variable(observer()).define(["md"], function(md){return(md)});
...
data.filter(d => d.name === "Heineken")
  ...
  //main.variable(observer()).define(["md","n"], function(md,n){return(md)});
...
rank(name => datevalues[0][1].get(name))
  ...
  main.variable(observer("k")).define("k", function(){return(10)});
...
function ticker(svg) {
  const now = svg.append("text")
      .style("font", `bold ${barSize} var(--sans-serif)`)
      .style("font-variant-numeric", "tabular-nums")
      .attr("text-anchor", "end")
      .attr("x", width - 30)
